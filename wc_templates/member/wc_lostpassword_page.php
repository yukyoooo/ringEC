					...
					<div class="whitebox">
						...
						<div class="loginbox">
							<form name="loginform" id="loginform" action="<?php usces_url( 'member' ); ?>" method="post" onKeyDown="if(event.keyCode == 13){return false;}">
								...
								<p class="submit"><input type="submit" name="lostpassword" id="member_login" value="<?php esc_attr_e( 'Obtain new password', 'usces' ); ?>" /></p>
								<?php do_action( 'usces_action_newpass_page_inform' ); ?>
							</form>

							<div><?php esc_html_e( 'Change your password by following the instruction in this mail.', 'usces' ); ?></div>

						<?php if ( ! usces_is_login() ) : ?>
							<p id="nav">
								<a href="<?php usces_url( 'login' ); ?>" title="<?php esc_attr_e( 'Log-in', 'usces' ); ?>">
